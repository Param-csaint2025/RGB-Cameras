...
Outputs:
    ...
    - Excel file ('object_speeds_depth.xlsx') with average lat, lon, speed, and direction for each object
...
CAMERA_INTRINSICS = {
    'fx': 1000, 'fy': 1000, 'cx': 960, 'cy': 540  # Example values for 1920x1080
}
FPS = 30                 # Frames per second

# --- PATHS ---
VIDEO_PATH = r'E:\RGB-cameras\WhatsApp Video 2025-07-09 at 18.40.05_339310d6.mp4'  # RGB video
...
class SimpleTracker:
    ...
    def update(self, detections, frame_idx):
        ...
        input_xyzs = np.array([d[0] for d in detections]) if detections else np.empty((0, 3))
...
def main():
    ...
    displays annotated video, and saves summary to Excel.
    """
    # Load YOLOv8 model
    ...
    frame_idx = 0
    object_data = defaultdict(lambda: {'lat': [], 'lon': [], 'speed': [], 'direction': []})
    prev_xyz = {}  # id -> previous world xyz
    while True:
        ret, frame = cap.read()
        if not ret:
            break
        ...
        if depth_cap:
            ...
            depth_map = cv2.cvtColor(depth_frame, cv2.COLOR_BGR2GRAY).astype(np.float32) / 255.0 * 50  # Example scaling
        else:
        ...
        for oid, (xyz, last_frame) in objects.items():
            lat, lon, alt = camera_to_world(xyz, CAMERA_LAT, CAMERA_LON, CAMERA_ALT, CAMERA_FACING_DEG)
            ...
            if len(traj) >= 2:
                ...
                direction = ''
            object_data[oid]['lat'].append(lat)
            object_data[oid]['lon'].append(lon)
            object_data[oid]['speed'].append(speed)
            object_data[oid]['direction'].append(direction)
            # Draw on frame
            label_parts = []
            if SHOW_ID:
            ...
            # Draw text
            cv2.putText(frame, label, (px, py-10), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0,255,255), 2)
    ...
    for oid, data in object_data.items():
        if data['lat'] and data['lon'] and data['speed']:
            ...
            # Use the mean direction in degrees, ignoring empty strings
            dir_values = [d for d in data['direction'] if isinstance(d, (int, float)) or (isinstance(d, str) and d != '')]
            if dir_values:
                avg_dir = sum([float(d) for d in dir_values]) / len(dir_values)
            else:
                avg_dir = ''
            rows.append({'id': oid, 'lat': avg_lat, 'lon': avg_lon, 'speed': avg_speed, 'direction': avg_dir})
    df = pd.DataFrame(rows, columns=['id', 'lat', 'lon', 'speed', 'direction'])
    df.to_excel('object_speeds_depth.xlsx', index=False)
    print('Excel file saved as object_speeds_depth.xlsx')
    cap.release()
    if depth_cap:
        depth_cap.release()
